...
- **Per-work-order production count** (2026-08-13, bugfix) — `g_andon.productionCount` used to be one value shared by every work order, so switching from WO A (just updated to some count) to WO B in the picker carried A's number into B's counter instead of showing B's own ("pindah2 wo list angkanya ttp sama yang terakhir di input"). `AndonWorkOrders` now tracks each work order's own count (`productionCount(idx)`/`setProductionCount(idx, count)`, NVS-keyed by that work order's id so it survives the list being re-fetched/reordered) - `onWorkOrderSelect()` reads it in, `onProductionConfirm()` writes it back, `setup()`'s boot-time push uses it too. Device-local only, same "not operator-owned truth" caveat as before - not fetched from `backend/`'s own per-WO production store (see that package's `production-store.ts`), so two terminals working the same work order can still diverge locally.
