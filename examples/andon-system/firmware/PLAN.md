...
- **Production count update** — `AndonMqtt::submitProductionUpdate()`, same accept/reject mechanics (ad-hoc extension, not in architectur.md §8 - see that file's own note). As of 2026-08-13 this also fires once automatically at boot (`setup()`, after the work-order sync below), not just on an operator's `CONFIRM` tap — see architectur.md §8.1's matching note.
- **Work order picker** (2026-08-13) — new `andon_workorders.cpp`/`SCR_WORK_ORDER_LIST`, same fetch/cache/placeholder-fallback shape as config sync: `GET /api/v1/work-orders/stations/{stationId}`, NVS-cached, last selection persisted and restored across reboots. Reached from SCR-01's UPDATE PRODUCTION button (picks a WO, then proceeds into the counter screen); replaces every hardcoded `"WO-240811-07"` literal in `main.cpp`/`andon_mqtt.cpp` calls with the operator's actual selection. UI reworked same day from direct-tap rows to an Up/Down cursor + Select button (mirrors `andon_wifi.cpp`'s network list) — explicit request ("jaga2 kalau lebih dr 3 listnya") so a station with more than `WORK_ORDER_LIST_VISIBLE_ROWS` (3) work orders still has a way to reach the rest, not just whatever fits on screen at once. See design.md's SCR-01 section and architectur.md §8.1 for the matching doc updates.
- **Per-work-order production count** (2026-08-13, bugfix, two passes) — `g_andon.productionCount` used to be one value shared by every work order, so switching from WO A (just updated to some count) to WO B in the picker carried A's number into B's counter instead of showing B's own ("pindah2 wo list angkanya ttp sama yang terakhir di input"). First pass: `AndonWorkOrders` tracked each work order's own count device-locally (NVS, keyed by work order id). **Superseded same day** ("ga bisa fetch dulu dr dashboard ya... jadi ngga sesuai dgn yang sudah di server") - device-local storage could drift from the backend/dashboard's own number with no reconciliation. Now `GET /api/v1/work-orders/stations/{stationId}` itself returns each work order's `productionCount` (`backend/src/server.ts` joins `work-order-store.ts`'s seed data with `production-store.ts`'s live state), and `AndonWorkOrders::sync()` is called both at boot AND every time the picker opens (`onOpenUpdateProduction()`, not just once at setup() like `AndonConfig::sync()`) - so the number the operator sees when picking a work order is the server's own, as of that exact moment, not a possibly-stale device cache. `setProductionCount(idx, count)` still exists but is in-memory only now (shows the operator's own just-confirmed number immediately, ahead of the next sync() round trip) - the server is durable storage, not this device.
- **Reject/defect counter** (2026-08-13) — `showScreenUpdateProduction()` gained a second, smaller REJECTS row (own +/- steppers, own counter) below the existing GOOD counter, purely so the dashboard can compute a real OEE Quality figure instead of assuming 100% (see `dashboard/PLAN.md`'s OEE entry). Same server-authoritative shape as `productionCount`: `AndonWorkOrders::rejectCount(idx)`/`setRejectCount(idx, count)`, fetched/refreshed by `sync()`, in-memory-only local writes. `AndonMqtt::submitProductionUpdate()` signature changed to take `rejectCount` too - payload gains `"rejectCount"` (see `architectur.md` §8.1's matching note); `g_andon.rejectCount` added to `AndonState`, preserved across the same reset points `productionCount`/`workOrderId` already were.
- **microSD card confirmed working** (2026-08-14, merged from branch `experiment/sdcard-test`) — "saya naruh sdcard didevice ini gimana taunya sdcardnya bisa digunakan". `main.cpp`'s `setup()` now runs `testSdCard()` once at boot (Serial-only diagnostic: card type/size + root directory listing, non-fatal if no card is present - the rest of boot proceeds either way). Two real bugs found and fixed along the way, both worth remembering for any future SD work on this board:
  1. **Global-SPI bus conflict** - `TFT_eSPI` (without `USE_HSPI_PORT`, which this firmware's `User_Setup.h` doesn't define) claims the *global* `SPI` object and remaps it onto the display's own pins (GPIO 12/13/14/15). Any `SD.begin()` sharing that same global object silently talks on the display's pins, not the TF slot's - looks exactly like "no card" (`Card Failed! cmd: 0x00`) even with a known-good card. Fix: SD gets its **own** `SPIClass(HSPI)` instance, explicitly `.begin()`'d on the TF slot's actual pins (`CS=5, MOSI=23, MISO=19, CLK=18` - confirmed straight off `docs/pcb-layout.jpg`'s silkscreen, independently corroborated by [this board family's RNT guide](https://randomnerdtutorials.com/esp32-cyd-display-touchscreen-microsd-card/)) before `SD.begin(TF_CS, thatSpiInstance, freq)`.
  2. **A merely-bad card can look identical to a wiring bug.** The original 4GB test card mounted fine in a PC's USB reader (independently verified: `lsblk` confirmed genuine healthy FAT32) but never responded in this board's slot at any SPI speed (4MHz or the SD spec's own 400kHz init minimum), even with the fix above applied and a bare-minimum isolation sketch (no TFT_eSPI/LVGL/GT911 at all) ruling out every other firmware variable - looked exactly like a dead slot. It wasn't: swapping in a different SDHC card worked immediately, first try, at 4MHz, everything else in the firmware still running normally alongside it. If a future card fails, don't assume the slot is broken until a second card's been tried.
...
- **SD-backed offline queue for Send Request/production updates** (2026-08-14, closes two of the reliability gaps below) — new `andon_offlinequeue.hpp`/`.cpp`: a simple SD-persisted FIFO of (topic, payload) pairs. `andon_mqtt.cpp`'s `publishEventAndAwaitResult()` gained a `queueOnFailure` param - on any "never got a definitive answer" exit (no WiFi, broker unreachable, or a COMMAND_RESULT timeout - explicitly NOT an explicit REJECTED result, which is a definitive answer, not a delivery failure) it hands the exact already-built envelope (idempotencyKey/eventId/correlationId all intact) to the queue instead of dropping it. `submitAndonRequest()`/`submitProductionUpdate()` pass `true`; `submitStatusUpdate()` (device-only Start Handling/Resolve) deliberately passes `false` - out of scope for automatic retry, per the same "technician must be physically at the terminal" reasoning as that feature's own product decision. `AndonMqtt::poll()` automatically resends the oldest queued entry (one per call, throttled to roughly every 8s) once MQTT is actually connected again - no operator action needed. Verified end-to-end on hardware: stopped `deploy/`'s backend, forced a production update (temporary Serial debug hook, since triggering this for real needs an operator tap and the touch-calibration screen blocks `setup()` on real taps every boot) - confirmed it queued (`AndonOfflineQueue: queued for retry`) instead of just failing; restarted the backend and confirmed the entry landed automatically (matching timestamp in `GET /api/v1/production`) with zero further interaction. Along the way, found and fixed a real bug: the queue-non-empty check was being evaluated every single `loop()` tick (not just inside the 8s throttle window), meaning `AndonOfflineQueue::count()`'s SD file open was happening ~20x/sec whenever MQTT was connected - reordered so the throttle's time check short-circuits before any SD I/O runs.

**Reliability gaps already flagged in `andon_mqtt.hpp`'s own scope note** (deliberate, for this MVP test slice — not oversights): persisted offline queue and idempotency-across-reboot are now DONE (see bullet above, `submitAndonRequest`/`submitProductionUpdate` only). Still missing: real bounded-exponential-backoff retry (the queue flush is a fixed ~8s interval, not backoff-with-jitter), and TLS/device credentials. All required for full production per agents.md §9/§12.
...
**Pending, not yet pulled into this repo:** an "andon fetch from API" piece reportedly done at the office, not yet pushed as of 2026-08-13. The work-order list itself is now done *here* too (see the bullet above) — the user separately mentioned a work-order feature was also built at the office; since both were developed independently without seeing each other, expect to reconcile shapes/endpoints (`GET /api/v1/work-orders/stations/{stationId}` here vs whatever the office side used) when that branch gets pulled, the same way `andon/v1/device/{deviceId}/state`'s shape was flagged provisional pending office reconciliation.
