...
- [examples/andon-system/](examples/andon-system/) — a multi-component "Digital Andon" shop-floor system (ESP32 terminal + backend + dashboard); `firmware/`, `backend/`, `contracts/`, `deploy/`, and `dashboard/` all exist now, only `docs/adr/` doesn't yet. This one has its own spec docs and agent instructions — see its section below before touching it.
...
3. **`examples/andon-system/firmware/src/main.cpp`** — the ESP32 terminal for the Digital Andon system (see below). GT911 driver, on-device touch calibration, and lvgl/TFT_eSPI plumbing carried over from `gemini-chatbot` (vendored lvgl 8.3.3 + TFT_eSPI again), plus design.md's numbered operator UI screens and several new-scope screens beyond it (production-count entry, work-order picker — mostly local state), a `WiFi`/`HTTPClient`/`ArduinoJson` config-sync module (`andon_config.cpp`/`.hpp`) that fetches per-category assistance reasons+codes from `backend/`'s config endpoint, a sibling work-order sync module (`andon_workorders.cpp`/`.hpp`, same cache/live-refresh/placeholder-fallback shape) that fetches the station's work-order list and persists the operator's selection across reboots (NVS-cache and hardcoded-placeholder fallbacks on both), an on-device WiFi setup screen (`andon_wifi.cpp`/`.hpp`) ported from `gemini-chatbot`'s WiFi manager (T9 keypad scan/select/password, NVS-saved network) — freely reachable from SCR-01's gear icon, a deliberate deviation from design.md's original admin-only intent (explicit user instruction, recorded in design.md) — and an MQTT client (`andon_mqtt.cpp`/`.hpp`, PubSubClient, QoS 0 only despite architectur.md asking for QoS 1) that both publishes (Send Request/`ANDON_REQUESTED`, the production-count screen's `PRODUCTION_COUNT_UPDATED` — also fired once automatically at every boot now, not just on an operator's confirm — and device-initiated `INCIDENT_STATUS_UPDATE` for Start Handling/Resolve, all ad-hoc extensions not in architectur.md - see that doc's §8.1 note) and subscribes (incident state pushed from the dashboard's Acknowledge action) on `backend/`, blocking briefly for each publish's `COMMAND_RESULT` before ever claiming backend-accepted. `setup()` also runs a Serial-only microSD diagnostic (`testSdCard()`) confirming the board's TF slot works — needs its own `SPIClass(HSPI)` instance explicitly bound to the TF pins (`CS=5, MOSI=23, MISO=19, CLK=18`), since `TFT_eSPI` otherwise claims the global `SPI` object for the display's pins and a shared `SD.begin()` silently talks to the wrong bus; see `examples/andon-system/firmware/PLAN.md`'s matching entry for the full story (including a card that looked like a dead slot but was just a bad card).
...
`firmware/` (UI + config sync + MQTT client, see above), `contracts/` (versioned HTTP contract for the config-sync endpoint, now implemented by `backend/`), `backend/` (Node/TS/Fastify + an embedded `aedes` MQTT broker — **test harness only**, in-memory, no auth/PostgreSQL, not the production backend `agents.md` §10 specs; see that package's `src/server.ts` header comment for the full scope note), `dashboard/` (Vite + vanilla TypeScript, no framework — live incident board + production tiles over a WebSocket feed from `backend/`, real Acknowledge/Start Handling/Resolve actions; see its own `PLAN.md` for scope/deviations), and `deploy/` (`docker-compose.yml` running both `backend/` on :8080 (HTTP)/:1883 (MQTT) and `dashboard/` on :8090 — `docker compose up --build` from that directory) all exist now.
